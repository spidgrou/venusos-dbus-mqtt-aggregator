...
import sys, os, platform, logging, time, dbus, traceback
...
class DbusMqttBridge:
    ...
    def __init__(self):
        ...
        self._find_services()
        self.system_id = self._get_dbus_value(self._dbus_paths['settings']['service'], "/Settings/System/VrmPortalId")
        if not self.system_id:
            logging.warning("VRM Portal ID not found. Falling back to device serial.")
            self.system_id = self._get_dbus_value(self._dbus_paths['system']['service'], "/Serial")
        if not self.system_id:
    ...
    def _get_dbus_value(self, service, path, default=None):
        try:
            ...
            if isinstance(val, (dbusInt16, dbusUInt16, dbusInt32, dbusUInt32, dbusByte)): return int(val)
            if isinstance(val, (dbusDouble,)): return float(val)
            if isinstance(val, (dbusString,)): return str(val)
            if isinstance(val, (dbusArray, list)): return default
            return default
        except Exception: return default

    def _get_display_name(self, service):
        return self._get_dbus_value(service, '/CustomName', default=self._get_dbus_value(service, '/ProductName', default=service))
    ...
    def _update_data(self):
        ...
        if sys_service: self.data['solar_power'] = self._get_dbus_value(sys_service, '/Dc/Pv/Power'); self.data['solar_current'] = self._get_dbus_value(sys_service, '/Dc/Pv/Current')
        total_yield = 0.0
        for charger in self._dbus_paths['solarchargers']:
            yield_kwh = self._get_dbus_value(charger['service'], '/History/Daily/0/Yield')
        ...
        if vebus_service:
            self.data['vebus_in_voltage'] = self._get_dbus_value(vebus_service, '/Ac/ActiveIn/L1/V'); self.data['vebus_in_frequency'] = self._get_dbus_value(vebus_service, '/Ac/ActiveIn/L1/F'); self.data['vebus_in_power'] = self._get_dbus_value(vebus_service, '/Ac/ActiveIn/L1/P'); self.data['vebus_out_voltage'] = self._get_dbus_value(vebus_service, '/Ac/Out/L1/V'); self.data['vebus_out_frequency'] = self._get_dbus_value(vebus_service, '/Ac/Out/L1/F'); self.data['vebus_out_power'] = self._get_dbus_value(vebus_service, '/Ac/Out/L1/P'); self.data['vebus_charge_current'] = self._get_dbus_value(vebus_service, '/Dc/0/Current')
