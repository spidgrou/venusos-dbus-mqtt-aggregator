...
class DbusMqttBridge:
    ...
    def _dbus_get_value(self, service, path, default=None):
        try:
            return self._dbus_conn.get_object(service, path).GetValue()
        except Exception as e:
            # logging.warning(f"Could not read D-Bus value for {service} at {path}: {e}")
            return default
    ...
    def _update_data(self):
        #  (il contenuto di questa funzione e invariato)
        ...
        if vebus_service: self.data['vebus_in_voltage'] = self._dbus_get_value(vebus_service, '/Ac/ActiveIn/L1/V'); self.data['vebus_in_frequency'] = self._dbus_get_value(vebus_service, '/Ac/ActiveIn/L1/F'); self.data['vebus_in_power'] = self._dbus_get_value(vebus_service, '/Ac/ActiveIn/L1/P'); self.data['vebus_out_voltage'] = self._dbus_get_value(vebus_service, '/Ac/Out/L1/V'); self.data['vebus_out_frequency'] = self._dbus_get_value(vebus_service, '/Ac/Out/L1/F'); self.data['vebus_out_power'] = self._dbus_get_value(vebus_service, '/Ac/Out/L1/P'); self.data['vebus_charge_current'] = self._dbus_get_value(vebus_service, '/Dc/0/Current'); state_code = self._dbus_get_value(vebus_service, '/State');
        if vebus_service and 'state_code' in locals() and state_code is not None: self.data['vebus_state_text'] = self.VEBUS_STATE_MAP.get(state_code, f'Unknown ({state_code})')
        return True
    ...
    def _publish_to_mqtt(self):
        ...
        for key, topic_suffix in topic_map.items():
            ...
            if value is not None:
                topic = f"Y/{self.system_id}/{topic_suffix}"; payload = round(value, 2) if isinstance(value, (int, float)) else value
                self.mqtt_client.publish(topic, payload, retain=True)

    def run(self):
        logging.info("--> Main loop tick: Reading data and publishing...")
        try:
            self._update_data()
